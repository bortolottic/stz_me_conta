...
class NPS extends React.Component {
	...
	renderNPSRegionalTable() {
		let NPSRegionalTableHCDefinition = {
			"qStateName": "",
			"qDimensions": [
			  {
				"qLibraryId": "BywxPaS",
		...
		this.props.app.createCube(NPSRegionalTableHCDefinition, (res) => {
			this.saveHypercubeId(res.qInfo.qId);
	...
	handleTableClick(params) {		
		this.props.props.history.push('/nps/'+params.value);
	}
	...
	render() {
		...
		let dropdownSkipPageOptions = [
			...
			{ name: "DATA", path: "nps/detalhe-dia"},
			];

		let dropdownSkipPage = <DropdownSkipPage activeNav="REG." options={dropdownSkipPageOptions} />
		
		if(this.state.percentage) {
			npsKpi = <FeaturedKPI color={this.state.percentage.color} number={this.state.percentage.value}  bottomElement={dropdownNav} app={this.props.app} descricaoFiltro={true} selectedFields={this.props.selectedFields} activeField={this.props.activeField} />
		}  else {
			npsKpi = <ObjectLoader></ObjectLoader>
		}

		if(this.state.npsTableData) {
			npsTable = <CustomTable _onRowClick={(props) => {this.handleTableClick(props)}} id="table1" data={this.state.npsTableData}></CustomTable>;
		} else {
			npsTable = <ObjectLoader></ObjectLoader>;
		}
		...
		return 	(
			<div class="page" ref={this.page} style={{heigth: '100%'}}>
				<HashRouter basename="/nps">
					<Switch>
						<Route path="/" exact  render={() => (page)} />
						<Route path="/detalhe-dia" exact render={(props) => {
							if(this.props.app) {
								return <NPSLojaPorDia app={this.props.app} props={props} isAuthed={true} directDetailDay={true} />
							}
						}}/>
						<Route path="/loja" exact render={(props) => {
							if(this.props.app) {
								return <NPSLoja app={this.props.app} props={props} isAuthed={true} directLoja={true} />
							}
						}}/>
						<Route path="/:regional" exact render={(props) => {
							if(this.props.app) {
								return <NPSLoja app={this.props.app} props={props} isAuthed={true} />
							}
						}}/>					
						<Route path="/:regional/detalhe-dia/:data" exact render={(props) => {
							if(this.props.app) {
								return <NPSMotivo app={this.props.app} props={props} isAuthed={true} />
							}
						}}/>
						<Route path="/:regional/detalhe-dia" exact render={(props) => {
							if(this.props.app) {
								return <NPSData app={this.props.app} props={props} isAuthed={true} dataSemLoja={true}/>
							}
						}}/>
						<Route path="/:regional/:loja" exact render={(props) => {
							if(this.props.app) {
								return <NPSData app={this.props.app} props={props} isAuthed={true}/>
							}
						}}/>
