...
class NPSData extends React.Component {
  ...
  renderNPSPercentage() {
      ...
      let data = res.qHyperCube.qDataPages[0].qMatrix[0][0];
			...
			this.setState({number: {value: number, color: 'white'}});
		});
	}
    ...
    let dropdownSkipPageOptions = [
      { name: "REG.", path: "/nps"},
      { name: "LOJA", path: "nps/"+this.props.props.match.params.regional},
      { name: "DATA", path: `/nps/${this.props.props.match.params.regional}/detalhe-dia`},
    ];
    ...
    //   { name: "REG.", path: "/nps"},
    //   { name: "LOJA", path: "nps/"+this.props.props.match.params.regional},
    //   { name: "DATA", path: "nps/loja/detalhe-dia"},
    // ];
    }
