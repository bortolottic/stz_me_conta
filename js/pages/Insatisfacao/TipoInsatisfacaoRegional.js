...
class TipoInsatisfacaoRegional extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.app.field('Motivos').selectMatch(tipo).then(() => {
            ...
            this.props.app.createCube(unsatisfiedChartHCDefinition, (res) => {
                this.saveHypercubeId(res.qInfo.qId);
                let unsatisfiedData = [['regiao', 'Porcentagem', 'Absoluto']];
				let values = [];
	...
	saveHypercubeId(id) {
		...
		this.props.props.history.push('/'+this.props.props.match.params.regional+"/"+params.name);
	}
	...
	render() {
		...
		let dropdownSkipPageOptions = [
			{ name: "REG.", path: "/"},
			{ name: "LOJA", path: `tipos-insatisfacao/${this.props.props.match.params.regional}/loja`},
			{ name: "DATA", path:'tipos-insatisfacao'+'/'+this.props.props.match.params.regional+"/detalhe-dia"},
			];
